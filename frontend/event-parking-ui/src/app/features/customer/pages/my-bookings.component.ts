...
@Component({
        ...
        @for(b of items();track b.bookingId){
          <article class="booking">
            ...
            <div class="details">
              <div class="title-row">
                <h3>{{b.eventName}}</h3>
                <span class="status" [class.confirmed]="b.status==='Confirmed'" [class.cancelled]="b.status==='Cancelled'" [class.expired]="b.status==='Expired'">{{b.status}}</span>
              </div>
              <p>◷ {{b.eventDate}} · {{b.startTime}}</p>
              <p>⌖ {{b.venueName}}</p>
              <div class="meta-row">
                <span>Booking <b>{{b.bookingNumber}}</b></span>
                @if(b.seatCount!=null){<span>Tickets <b>{{b.seatCount}}</b></span>}
                @if(b.parkingSlot){<span>Parking <b>{{b.parkingSlot}}</b></span>}
              </div>
            </div>
            ...
            <div class="right">
              ...
              @if(b.status==='Confirmed'){
                <button class="qr-btn" type="button" [disabled]="downloadingId()===b.bookingId" (click)="downloadQr(b)">
                  {{downloadingId()===b.bookingId ? 'Preparing QR...' : '⬇ Download QR'}}
                </button>
                <button class="cancel-btn" (click)="cancel(b.bookingId)">Cancel booking</button>
              }
            </div>
          </article>
        } @empty {
  ...
  styles:[`
    ...
    .right{text-align:right;display:flex;flex-direction:column;gap:6px;align-items:flex-end}.right small{font-size:7px;color:#809196}.right>b{font-size:16px;color:#0e8f79}.right button{border-radius:8px;padding:7px 9px;font-size:7px;font-weight:900;cursor:pointer}.qr-btn{margin-top:5px;border:1px solid #9dc5f2;background:#eef6ff;color:#0b66dc}.qr-btn:disabled{opacity:.55;cursor:wait}.cancel-btn{border:1px solid #efb4b4;background:#fff1f1;color:#b72b2b}
...
export class MyBookingsComponent{
  ...
  load(t:string){
    this.tab.set(t);
    this.api.bookings(t).subscribe({
      next:x=>{this.items.set(x||[]);this.error.set('')},
      error:()=>{this.items.set([]);this.error.set('Could not load bookings. Make sure the backend is running and you are signed in as a Customer.')}
    })
  }

  downloadQr(b:any){
    this.error.set('');
    this.downloadingId.set(b.bookingId);

    this.api.qr(b.bookingId).subscribe({
      next:(blob:Blob)=>{
        const url=URL.createObjectURL(blob);
        const a=document.createElement('a');
        a.href=url;
        a.download=`EventPark-${b.bookingNumber || 'booking-'+b.bookingId}-QR.png`;
      ...
      error:e=>{
        this.downloadingId.set(null);
        this.error.set(e?.error?.message || 'Could not download this QR ticket.')
      }
    })
  }
  ...
  cancel(id:number){
    if(confirm('Cancel this booking?')){
      this.api.cancel(id).subscribe({
        next:()=>this.load(this.tab()),
        error:e=>this.error.set(e?.error?.message??'Cancellation failed. Please try again.')
      })
    }
  }
}
